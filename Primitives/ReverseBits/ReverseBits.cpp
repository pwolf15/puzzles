...
public:
    uint32_t reverseBits(uint32_t n)
    {
        if (!initCache)
        {
            size_t shortMax = std::numeric_limits<unsigned char>::max();
            reversed.resize(shortMax + 1);
            for (size_t num = 0; num < reversed.size(); num++)
            {
        ...
        // complexity is O(n / L) where n is n-bit integer and L is number of bits in cached keys
        return ((long long)reversed[(n >> maskSize * 3) & mask]) |
               (((long long)reversed[(n >> maskSize * 2) & mask]) << maskSize) |
               (((long long)reversed[(n >> maskSize) & mask]) << maskSize * 2) |
               (((long long)reversed[n & mask]) << maskSize * 3);
    }
private:
...
int main()
{
    ...
    Solution sol;
    std::cout << std::hex << sol.reverseBits(0x1) << std::dec << std::endl;
